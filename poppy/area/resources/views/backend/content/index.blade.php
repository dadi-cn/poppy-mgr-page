			...
			@if ($items->total())
				@foreach($items as $item)
					<tr>
						<td>{{$item->id}}</td>
						<td>
							@if($item->parent_id && $item->level)
								<a href="{{ route_url('py-area:backend.content.index',['id'=>$item->id]) }}" class="text-success">
									{{$item->title}}
								</a>
							@else
								{{$item->title}}
						...
						<td>
							...
							<a title="删除" class="J_request J_tooltip"
								...
								href="{{route('area:backend.content.delete', [$item->id])}}">
