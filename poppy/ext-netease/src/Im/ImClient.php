...
class ImClient
{
    public const SEX_MALE   = 1;  //男士
    public const SEX_FEMALE = 2;  //女士

    public const FRIEND_BLACK = 1;  // 黑名单
    public const FRIEND_MUTE  = 2;  // 静音
    ...
    /**
     * @var string [SHA1](AppSecret + Nonce + CurTime),三个参数拼接的字符串，进行SHA1哈希计算，转化成16进制字符(String，小写)
     */
    private $checkSum;
     ...
     * 创建云信ID
     * 1.第三方帐号导入到云信平台；
     * 2.注意 accid，name长度以及考虑管理秘钥token
     * @param $data
     * @return array $result    [返回array数组对象]
     */
    public function createUserIds($data): array
    {
     ...
     * @return array $result [返回array数组对象]
     */
    public function updateUserId($data): array
    {
     ...
     * @return array $result  [返回array数组对象]
     */
    public function blockUserId($accid): array
    {
     ...
     * @return array $result  [返回array数组对象]
     */
    public function unblockUserId($accid): array
    {
     ...
     *  string     $icon   [用户icon，最大长度256字节]
     *  string     $sign   [用户签名，最大长度256字节]
     *  string     $email  [用户email，最大长度64字节]
     *  string     $birth  [用户生日，最大长度16字节]
     *  string     $mobile [用户mobile，最大长度32字节]
     *  int|string $gender [用户性别，0表示未知，1表示男，2女表示女，其它会报参数错误]
     *  string     $ex     [用户名片扩展字段，最大长度1024字节，用户可自行扩展，建议封装成JSON字符串]
     * @return array $result      [返回array数组对象]
     */
     ...
     * @return array $result  [返回array数组对象]
     */
    public function getUserInfos($accids): array
    {
    ...
    /**
     * 获取单用户的信息, 多个用户获取还是使用 getUserInfos
     * @param string $accid accID
     * @return array|mixed
     */
    public function getUserInfo($accid): array
    {
        $result    = $this->getUserInfos([$accid]);
        $userInfos = $result['uinfos'] ?? [];
        return $userInfos[0] ?? [];
    }
     ...
     * @return array $result      [返回array数组对象]
     */
    public function addFriend($accid, $faccid, $type = '1', $msg = ''): array
    {
     ...
     * @return array $result      [返回array数组对象]
     */
    public function updateFriend($accid, $faccid, $alias): array
    {
     ...
     * @return array $result   [返回array数组对象]
     */
    public function getFriend($accid): array
    {
     ...
     * @return array $result      [返回array数组对象]
     */
    public function deleteFriend($accid, $faccid): array
    {
     ...
     * @return array $result        [返回array数组对象]
     */
    public function blackFriend($accid, $targetAcc, $relationType = self::FRIEND_BLACK, $value = '1'): array
    {
     ...
     * @return array $result  [返回array数组对象]
     */
    public function listBlackFriend($accid): array
    {
    ...
    public function sendBatchMsg($data): array
    {
     ...
     * @return array $result            [返回array数组对象]
     */
    public function sendAttachMsg($data): array
    {
     ...
     * @return array $result  [返回array数组对象]
     */
    public function uploadMsg($content, $type = '0'): array
    {
     ...
     * @return array     $result  [返回array数组对象]
     */
    public function uploadMultiMsg($content, $type = '0'): array
    {
     ...
     * 群组功能（高级群）-创建群
     * @param $data
     *      string     $tname        [群名称，最大长度64字节]
     *      string     $owner        [群主用户帐号，最大长度32字节]
     *      string     $members      [["aaa","bbb"](JsonArray对应的accid，如果解析出错会报414)，长度最大1024字节]
     *      string     $announcement [群公告，最大长度1024字节]
     *      string     $intro        [群描述，最大长度512字节]
     *      string     $msg          [邀请发送的文字，最大长度150字节]
     *      int|string $magree       [管理后台建群时，0不需要被邀请人同意加入群，1需要被邀请人同意才可以加入群。其它会返回414。]
     *      int|string $joinmode     [群建好后，sdk操作时，0不用验证，1需要验证,2不允许任何人加入。其它返回414]
     *      string     $custom       [自定义高级群扩展属性，第三方可以跟据此属性自定义扩展自己的群属性。（建议为json）,最大长度1024字节.]
     * @return array $result           [返回array数组对象]
     */
     ...
     */
    public function addIntoGroup($tid, $owner, $members, $magree = '0', $msg = '请您入伙'): array
    {
     ...
     */
    public function kickFromGroup($tid, $owner, $member): array
    {
     ...
     * 群组功能（高级群）-解散群
     * @param array $data
     *        string tid   [云信服务器产生，群唯一标识，创建群时会返回，最大长度128字节]
     *        string owner [群主用户帐号，最大长度32字节]
     * @return array       [返回array数组对象]
     */
    public function removeGroup($data): array
    {
     ...
     * @return array               [返回array数组对象]
     */
    public function updateGroup($tid, $owner, $tname, $announcement = '', $intro = '', $joinmode = '0', $custom = ''): array
    {
     ...
     * @return array       [返回array数组对象]
     */
    public function queryGroup($tids, $ope = '1'): array
    {
    ...
    public function queryDetail($data): array
    {
     ...
     * @return array           [返回array数组对象]
     */
    public function changeGroupOwner($tid, $owner, $newowner, $leave = '2'): array
    {
     ...
     * @return array          [返回array数组对象]
     */
    public function addGroupManager($tid, $owner, $members): array
    {
     ...
     * @return array          [返回array数组对象]
     */
    public function removeGroupManager($tid, $owner, $members): array
    {
     ...
     * @return array        [返回array数组对象]
     */
    public function joinTeams($accid): array
    {
     ...
     * @return array        [返回array数组对象]
     */
    public function updateGroupNick($tid, $owner, $accid, $nick): array
    {
     ...
     *    string $reverse   [1按时间正序排列，2按时间降序排列。其它返回参数414.默认是按降序排列。]
     * @return array        [返回array数组对象]
     */
     ...
     * @return array            [返回array数组对象]
     */
    public function queryGroupMsg($tid, $accid, $begintime, $endtime = '', $limit = '100', $reverse = '1'): array
    {
     ...
     * @return array           [返回array数组对象]
     */
    public function sendSmsCode($mobile, $deviceId = ''): array
    {
     ...
     * @return array            [返回array数组对象]
     */
    public function sendSMSTemplate($templateid, $mobiles = [], $params = []): array
    {
     ...
     * @return array         [返回array数组对象]
     */
    public function querySMSStatus($sendid): array
    {
     ...
     * @return array            [返回array数组对象]
     */
    public function startCall($callerAcc, $caller, $callee, $maxDur = '60'): array
    {
     ...
     * @return array            [返回array数组对象]
     */
    public function startConf($callerAcc, $caller, $callee, $maxDur = '60'): array
    {
     ...
     * @return array          [返回array数组对象]
     */
    public function queryCallsBySession($session, $type): array
    {
     ...
     * @return array
     */
    public function getUserSignature($uid): array
    {
     ...
     * @return array
     */
    public function channelCreate($name, $type): array
    {
     ...
     * @return array
     */
    public function channelUpdate($name, $cid, $type): array
    {
     ...
     * @return array
     */
    public function channelDelete($cid): array
    {
     ...
     * @return array
     */
    public function channelStats($cid): array
    {
     ...
     * @return array
     */
    public function channelList($records = 10, $pnum = 1, $ofield = 'ctime', $sort = 0): array
    {
     ...
     * @return array
     */
    public function channelRefreshAddr($cid): array
    {
     ...
     * @return array
     */
    public function chatRoomCreates($data): array
    {
     ...
     * @return array
     */
    public function chatRoomUpdates($roomid, $name): array
    {
     ...
     */
    public function chatRoomToggleCloses($roomid, $operator): array
    {
     ...
     * @return array
     */
    public function chatRoomToggleStats($roomid, $operator): array
    {
        $url  = 'https://api.netease.im/nimserver/chatroom/toggleCloseStat.action';
        $data = [
            'roomid'   => $roomid,
     ...
     * @return array
     */
    public function chatRoomSetMemberRoles($roomid, $operator, $target, $opt, $optvalue): array
    {
     ...
     * @return array
     */
    public function chatRoomGets($data): array
    {
     ...
     * @return array
     */
    public function leaveGroupManager($data): array
    {
    ...
    /**
     * 撤回消息
     * @param array $data 撤回消息内容
     * @return array
     */
    public function recall($data): array
    {
     ...
     */
    private function postJsonDataCurl($url, $data): array
    {
     ...
     * @return array $请求返回结果(array)
     */
    private function postJsonDataFsockopen($url, $data): array
    {
        $this->createCheckSum();       //发送请求前需先生成checkSum
    ...
    /**
     * 将json字符串转化成php数组
     * @param string|array|object $json_str
     * @return array
     */
    private function jsonToArray($json_str): array
    {
        if (is_array($json_str) || is_object($json_str)) {
            $is_json = $json_str;
        }
        elseif (is_null(json_decode($json_str, true))) {
            $is_json = $json_str;
        }
        else {
            $is_json = (string) $json_str;
     ...
     * @return array $请求返回结果(array)
     */
    private function postDataCurl($url, $data): array
    {
        $this->createCheckSum();       //发送请求前需先生成checkSum
     ...
     * @return array $请求返回结果(array)
     */
    private function postDataFsockopen($url, $data): array
    {
        $this->createCheckSum();       //发送请求前需先生成checkSum
