    ...
    <div class="container navbar-expand-lg">
        ...
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
            <ul class="navbar-nav mr-auto">
                <li class="nav-item {!! active_class(if_route(['misc:tool.index'])) !!}">
                    <a class="nav-link" href="{!! route('misc:tool.index') !!}">工具</a>
                </li>
                <li class="nav-item {!! active_class(if_route(['misc:nav.index'])) !!}">
        ...
        <ul class="navbar-nav ml-auto">
            <li class="nav-item {!! active_class(if_route(['slt:nav.index'])) !!}">
                <a class="nav-link J_iframe" data-width="320" data-height="480" href="{!! route_url('misc:user.login') !!}">
                    <i class="fa fa-user"></i> @if ($_pam) {!! $_pam->username !!} @endif
                </a>
            </li>
            @if ($_pam)
                <li class="nav-item {!! active_class(if_route(['misc:user.logout'])) !!}">
                    <a class="nav-link" href="{!! route('misc:user.logout') !!}">
